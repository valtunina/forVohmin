...
├── Snakefile # Главный файл workflow
├── config.yaml # Конфигурация (URL книги)
├── scripts/
│ └── count_words.py # Python-скрипт для подсчёта слов
├── data/ # Входные данные
└── results/ # Результаты

## 🚀 Как запустить проект

### Вариант 1: GitHub Codespaces (самый простой, онлайн)
...
3. В открывшемся терминале выполните:
   ```bash
   snakemake --cores 1
Вариант 2: Через Docker (рекомендуется для локального запуска)
bash
